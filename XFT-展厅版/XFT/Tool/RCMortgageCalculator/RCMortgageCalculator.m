...
@interface RCMortgageCalculator ()

@end

@implementation RCMortgageCalculator
#pragma mark - 非组合贷
/// 非组合贷 计算方法
/// @param rate 年利率
...
+ (RCCalculateResultModel *)calculateMortgageInfoWithYearRate:(CGFloat)rate
    ...
    // 创建 计算结果 model
    RCCalculateResultModel *resultModel = nil;
...
+ (RCCalculateResultModel *)zuheDaiCalculateWithGJJYearRate:(CGFloat)gjjYearRate
    ...
    NSMutableArray <RCCalculateResultMonthModel *>* monthMDArray = [NSMutableArray array];
    
    // 创建 计算结果 model
    RCCalculateResultModel *resultModel = [[RCCalculateResultModel alloc] init];
    ...
    if (calculateMethod == CalculationMethod_BenXi) {
        ...
        // 分别按各自的贷款利率
        RCCalculateResultModel *gjjModel = [self benxiCalculateWithMonthRate:gjjMonthRate monthCount:monthCount totalMoney:gjjMoneyYuan];
        RCCalculateResultModel *shangModel = [self benxiCalculateWithMonthRate:shangMonthRate monthCount:monthCount totalMoney:shangMoneyYuan];
        ...
        for (int i = 0; i < gjjModel.monthResultArray.count; i++) {
            ...
            //
            RCCalculateResultMonthModel *gjjMonthModel = gjjModel.monthResultArray[i];
            RCCalculateResultMonthModel *shangMonthModel = shangModel.monthResultArray[i];
                
            RCCalculateResultMonthModel *monthModel = [[RCCalculateResultMonthModel alloc] init];
        ...
        // 分别按各自的贷款利率
        RCCalculateResultModel *gjjModel = [self benjinCalculateWithMonthRate:gjjMonthRate monthCount:monthCount totalMoney:gjjMoneyYuan];
        RCCalculateResultModel *shangModel = [self benjinCalculateWithMonthRate:shangMonthRate monthCount:monthCount totalMoney:shangMoneyYuan];
...
+ (RCCalculateResultModel *)benxiCalculateWithMonthRate:(CGFloat)rate monthCount:(NSUInteger)monthCount totalMoney:(CGFloat)totalMoney {
...
+ (RCCalculateResultModel *)benjinCalculateWithMonthRate:(CGFloat)rate monthCount:(NSUInteger)monthCount totalMoney:(CGFloat)totalMoney {
