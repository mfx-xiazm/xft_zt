...
- (void)setUpAllTypeAvatarGroupId:(NSString *)groupId Source:(NSArray <NSString *>*)groupSource itemPlaceholder:(id)placeholder options:(DCGroupAvatarCacheType)options setImageBlock:(GroupSetImageBlock)setImageBlock completed:(GroupImageBlock)completedBlock
{
    ...
    [DCAvatarHelper dc_fetchLoadImageSource:groupSource cacheGroupImage:(UIImage *)groupImage itemPlaceholder:placeholder completedBlock:^(NSArray <UIImage *>*unitImages, BOOL succeed) {
        ...
        if (avatarType == DCGroupAvatarWeiBoType) { // WeiBo
        ...
        }else if (avatarType == DCGroupAvatarWeChatType || avatarType == DCGroupAvatarQQType) { // WeChat / QQ
            ...
            // 小图像的大小
            itemAvaSize = CGSizeMake(containerSize.height, containerSize.height);
            // 小图像的半径
            CGFloat radius = containerSize.height/2.0;
            // 小图像边圈宽度
            __block CGFloat bordWidth = [DCAvatarManager sharedAvatar].bordWidth;
            ...
            for (NSInteger i = 0; i < groupUnitImages.count; ++i) {
                @autoreleasepool {
                    ...
                    itemAvaPoint = CGPointMake(radius+containerSize.height*i-5*i, radius);
